...
if __name__ == '__main__':
    ...
    DATA_DIR   = os.path.join(ROOT, 'data', 'Case_Study')
    RESULTS_DIR= os.path.join(ROOT, 'data', 'Results', 'Resultados_CCO')
    os.makedirs(RESULTS_DIR, exist_ok=True)

    # --- Configure instance here ---
    excel_file = 'Case_Study_05.xlsx'   # Options: Case_Study_05/10/15.xlsx
    error_val  = '05'
    prob_val   = 95
    z_val      = 1.645                  # Phi^{-1}(0.95)
    # --------------------------------

    print(f"Error {error_val}% | Confidence {prob_val}% | Z = {z_val}")

    model_instance = P2P_Model(phi_inv=z_val)
    model_instance.ReadExcelFile(os.path.join(DATA_DIR, excel_file))
    modelo = model_instance.Solver()

    nombre_salida = os.path.join(RESULTS_DIR, f'Results_Error{error_val}_Conf{prob_val}_Single.xlsx')
    model_instance.ExportResults(modelo, output_name=nombre_salida)

    resumen = model_instance.get_summary_dict(modelo, error_val, prob_val)
    print("\n===== RESULTS SUMMARY =====")
    for key, val in resumen.items():
        print(f"  {key}: {val}")
    print(f"\nResults saved to: {nombre_salida}")
